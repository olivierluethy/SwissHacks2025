...
def create_html_from_workbook(filepath):
    """
    Process each sheet in the workbook to extract subtables.
    Creates clean HTML without styling for AI parsing.
    """
    try:
...
def create_markdown_from_workbook(filepath):
    """
    Process each sheet in the workbook to extract subtables.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract subtables from an Excel file with enhanced handling of merged cells, titles, and footers."
    )
    parser.add_argument("input_file", help="Path to the input Excel file (.xlsx)")
    parser.add_argument("output_file", help="Path for the output file (.html or .md)")
    args = parser.parse_args()

    # Determine output format based on file extension
    _, file_extension = os.path.splitext(args.output_file.lower())

    if file_extension == ".md":
        # Generate Markdown output
        final_output = create_markdown_from_workbook(args.input_file)
    else:
        # Default to HTML output
        final_output = create_html_from_workbook(args.input_file)

    with open(args.output_file, "w", encoding="utf-8") as f:
        f.write(final_output)

    print(f"Extraction complete. Output saved to {args.output_file}")
    print(f"Format: {'Markdown' if file_extension == '.md' else 'HTML'}")
